...
class RegressionType(knext.EnumParameterOptions):
    ...
    OLS = ("OLS", "Ordinary Least Squares - standard unregularized linear regression")
    RIDGE = ("Ridge", "Ridge (L2) regularization - shrinks coefficients towards zero. Optimal alpha selected via cross-validation.")
    LASSO = ("Lasso", "Lasso (L1) regularization - performs variable selection by setting some coefficients to zero. Optimal alpha selected via cross-validation.")
    PANEL = ("Panel Data", "Panel regression with entity and time fixed or random effects using linearmodels. Requires both entity and time index columns.")
...
class ModelSettings:
    ...
    include_intercept = knext.BoolParameter(
        ...
        knext.OneOf(regression_type, [RegressionType.PANEL.name]), knext.Effect.HIDE
    )
    ...
    compute_vif = knext.BoolParameter(
        ...
        knext.OneOf(regression_type, [RegressionType.RIDGE.name, RegressionType.LASSO.name, RegressionType.PANEL.name]), knext.Effect.HIDE
    )
...
class LinearModelLearner:
    ...
    **OLS/Ridge/Lasso:**
    ...
    def configure(
        ...
        coef_columns = ["Variable", "Estimate", "Standardized Beta", "Std Error", "t-statistic", "p-value", "CI Lower", "CI Upper", "VIF", "VIF Assessment"]
    ...
    def execute(self, exec_context: knext.ExecutionContext, input_table: knext.Table):
        ...
        if reg_type == RegressionType.OLS.name:
            ...
            results, beta, se_beta, t_stats, p_values, y_pred, residuals, predictor_names, selected_alpha = \
                self._fit_ols_model(X, y, predictor_names_for_model, sm, np)
            ...
            model, beta, se_beta, t_stats, p_values, y_pred, residuals, predictor_names, X_design, selected_alpha = \
                self._fit_regularized_model(X, y, predictor_names_for_model, np)
            ci_lower, ci_upper = None, None  # Will compute below
        ...
        cooks_d, leverage, flags = self._compute_influence_diagnostics(
            residuals, standardized_residuals, leverage, n_obs, beta, mse, np
        )
        ...
        buf = self._create_diagnostic_plots(y_pred, residuals, standardized_residuals, 
                                            cooks_d, leverage, n_obs, plt, stats, np)
        ...
        model_dict = {
            ...
            "model_object": model if reg_type in [RegressionType.RIDGE.name, RegressionType.LASSO.name] else None,  # sklearn fitted models (Ridge/Lasso only)
            "results_object": results if reg_type in [RegressionType.OLS.name, RegressionType.PANEL.name] else None,  # statsmodels/linearmodels fitted results (OLS/Panel only)
        }
    ...
    def _encode_categorical_variables(self, df_clean, pd):
        ...
        if categorical_cols:
            df_dummies = pd.get_dummies(df_clean[self.predictor_columns], 
    ...
    def _fit_panel_model(self, df, y, pd, np):
        ...
        if categorical_pred:
            df_dummies = pd.get_dummies(df_panel[predictors], 
                                       columns=categorical_pred, 
                                       drop_first=True,
                                       dtype=float)
        ...
        return (
            results, beta, se_beta, t_stats, p_values, ci_lower, ci_upper,
            y_pred, residuals, predictor_names, X_panel.values, y_panel.values, np.nan
        )
    ...
    def _build_model_summary(self, n_obs, n_predictors, stats_dict, selected_alpha, np):
        ...
        if reg_type == RegressionType.OLS.name:
            model_summary_data.extend([
                ("F-statistic", stats_dict['f_statistic'], 
    ...
    def _create_diagnostic_plots(self, y_pred, residuals, standardized_residuals, 
                                 cooks_d, leverage, n_obs, plt, stats, np):
        """
        ...
        ax5 = axes[1, 1]
        ax5.hist(standardized_residuals, bins=min(30, n_obs // 5), density=True, alpha=0.7, 
        ...
        if self.model_settings.compute_influence and reg_type == RegressionType.OLS.name:
            ...
            colors = ['red' if (l > leverage_threshold and abs(sr) > 2) else 'blue' 
                     for l, sr in zip(leverage, standardized_residuals)]
